...
def forced_line(cluster, exposures, times):
    """
    Perform forced photometry at each point in a line given a set of exposures and their times
    """
    ...
    for position, exposure in zip(positions, exposures):
        point = exposure.wcs.skyToPixel(lsst.geom.SpherePoint(position[0].to(u.deg).value, position[1].to(u.deg).value, units=lsst.geom.degrees))
        points.append(dict(
            i_x=point.getX(),
            i_y=point.getY(),
            ra=position[0].to(u.deg).value,
            dec=position[1].to(u.deg).value,
        ))
    points = astropy.table.Table(points)
        
    photometry = forced_exposures(exposures, points)
...
def main():
    ...
    photometry = forced_line(cluster, cutouts, times)
    photometry['clipped'] = clipped
    cluster.extra['forced_line'] = photometry
    cluster.extra['cutouts'] = cutouts
    cluster.extra['clipped'] = clipped
